package LC3661_Maximum_Walls_Destroyed_by_Robots;
import java.util.*;
class Solution {
    public int maxWalls(int[] robots, int[] distance, int[] walls) {
        ...
        for(int i=0; i<n; i++)  map.put(robots[i], distance[i]);
        Arrays.sort(robots);
        Arrays.sort(walls);
        for(int i=0; i<n; i++){
            ...
            int leftPos = 0;
            if(i >= 1){
                int leftBound = Math.max(robots[i] - map.get(robots[i]), robots[i-1] + 1);
                leftPos = lowerBound(walls, leftBound);
            }
            else{
                leftPos = lowerBound(walls, robots[i] - map.get(robots[i]));
            }
            left[i] = pos1 - leftPos;
            int rightPos = 0;
            if(i < n-1){
                int rightBound = Math.min(robots[i] + map.get(robots[i]), robots[i+1] - 1);
                rightPos = upperBound(walls, rightBound);
            }
            else{
                rightPos = upperBound(walls, robots[i] + map.get(robots[i]));
            }
